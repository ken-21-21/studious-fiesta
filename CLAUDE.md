...
- **Antigravity** — the user runs it separately against their local copy, on
  its own branch **`ANTILOG`**. Never push to `ANTILOG` myself.
...
3. **Merge strategy.** Merge (or cherry-pick) `ANTILOG` into mine — never the
   reverse, and never force-push over `ANTILOG`. If my branch has diverged
   since the last sync, rebase my unmerged work on top of the merge result so
   history stays linear. Resolve conflicts in favor of Antigravity's newer
   intent unless it breaks an invariant above.
